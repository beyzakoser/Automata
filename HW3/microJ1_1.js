...
class Method extends Node {
  ...
  toString() {
     ...
     for (let i=0; i<this.nParams; i++){
     	...
     	if (myMap != null && myMap.get(this.vars[i]) !=undefined){
	        a.push(this.vars[i]+" = "+myMap.get(this.vars[i]))
...
class Declaration {
   ...
   toString() { 
    ...
    for (let i =0; i <this.vars.length; i++) {
      let degerler = this.vars[i].val; 
      if(degerler!= null){
        this.vars[i]=this.vars[i]+" = " +this.vars[i].val;
      }
     } 
...
function method()  {
    ...
    if (tok.kind == IDENT) {
    	let paramlist=identList(); //metodun içindeki parametse sayısı 
   
   		met.nParams = met.vars.length; 
    	for(let i =0;i<paramlist.length;i++){

        myMap.set(met.vars[i],paramlist[i].val); //Map(3) {"c" => 7, "d" => 3, "g" => undefined}
